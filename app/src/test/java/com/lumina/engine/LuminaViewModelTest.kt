...
class LuminaViewModelTest {
    ...
    fun `processUserInput updates state with parsed intent`() = runTest {
        ...
        viewModel.luminaState.test {
            ...
            // Skip processing states, get to final
            val states = mutableListOf<LuminaState>()
            repeat(3) { 
                try {
                    states.add(awaitItem())
                } catch (e: Exception) {
                    // May not have that many emissions
                }
            }
            
            val final = states.lastOrNull() ?: viewModel.luminaState.value
    ...
    fun `processUserInput handles orchestrator exception`() = runTest {
        ...
        viewModel.statusMessage.test {
            ...
            // Should contain error message eventually
            val messages = mutableListOf<String>()
            repeat(3) {
                try {
                    messages.add(awaitItem())
                } catch (e: Exception) {
                    break
                }
            }
            
            assertThat(messages.any { it.contains("Error") }).isTrue()
        }
    }
